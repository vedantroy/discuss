...
import { runQuery } from "./utils/executor";
...
export const removeVote = async (
    { userId, votableUUID }: { userId: ShortUserID; votableUUID: string },
) => runQuery(e.delete(e.Vote, vote => ({
    filter: e.op(
        e.op(vote.user.shortId, "=", userId),
        "and",
        e.op(vote.votable.id, "=", e.uuid(votableUUID)),
    ),
})));

export const createVote = async (
    { userId, up, votableUUID }: {
        userId: ShortUserID;
        up: boolean;
        votableUUID: string;
    },
    ...
    e.insert(e.Vote, {
        ...
        votable: e.select(
            e.Votable,
            votable => ({ filter: e.op(votable.id, "=", e.uuid(votableUUID)) }),
        ),
    }).unlessConflict(vote => ({
